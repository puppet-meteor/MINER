...
class PayloadBodyChecker(CheckerBase):
    ...
    def apply(self, rendered_sequence, lock):
       ...
       # print('definition:'+str(last_request._definition))
    ...
    def _run_body_value_fuzzing(self, request, body_schema, config, tracker):
        ...
        #print('body blocks:'+str(body_blocks_fuzzed))
        for body_blocks in body_blocks_fuzzed:
    ...
    def _refresh(self, last_request):
        ...
        initial_response, response_to_parse = self._render_and_send_data(new_seq, last_request, True, 1)

        if initial_response.status_code[0] == '2':
            datacollect.payload_body_checker_laster_200 += 1
        elif initial_response.status_code[0] == '5':
            datacollect.payload_body_checker_laster_500 += 1
        elif initial_response.status_code[0] == '4':
            datacollect.payload_body_checker_laster_400 += 1
    ...
    def _execute_start_of_sequence(self):
        ...
        for request in self._sequence.requests[:-1]:
            ...
            initial_response, response_to_parse = self._render_and_send_data(new_seq, request, False)
            if initial_response.status_code[0] == '2':
                datacollect.payload_body_checker_former_200 += 1
            elif initial_response.status_code[0] == '5':
                datacollect.payload_body_checker_former_500 += 1
            elif initial_response.status_code[0] == '4': 
                datacollect.payload_body_checker_former_400 += 1

            # Check to make sure a bug wasn't uncovered while executing the
            # sequence
            if initial_response:
                if initial_response.status_code[0] == '5':
                    has_500 = True
                    has_200 = False
                elif initial_response.status_code[0] == '4':
                    has_400 = True
                ...
                if has_500 and not has_400:
                    datacollect.payload_body_checker_part_500 += 1
                elif has_400 and not has_500:
                    datacollect.payload_body_checker_part_400 += 1
                elif has_400 and has_500:
                    datacollect.payload_body_checker_part_non200 += 1
                elif has_200:
                    datacollect.payload_body_checker_part_200 += 1
        ...
        if has_500 and not has_400:
            datacollect.payload_body_checker_overall_500 += 1
        elif has_400 and not has_500:
            datacollect.payload_body_checker_overall_400 += 1
        elif has_400 and has_500:
            datacollect.payload_body_checker_overall_non200 += 1
        elif has_200:
            datacollect.payload_body_checker_overall_200 += 1 
    ...
    def _exec_request_with_new_body(
        ...
        for rendered_data, parser, list_values, list_values_default, whethergen in new_request.render_iter_lyu_atten(self._req_collection.candidate_values_pool, 1):
            # check time budget
