...
def extend(seq_collection, fuzzing_requests, lock):
    ...
    datacollect.tmptmpcount += 1
    rand_select_one = [] 
    for req in fuzzing_requests:
        for i in range(prev_len):
            if (datetime.datetime.now() - datacollect.starttime).seconds > 3600 and random.random() > math.log((seq_collection[i]).length + 1, 10):
                continue
            ...
            tmprequestendpoint = req.endpoint_no_dynamic_objects
            if tmprequestendpoint in datacollect.writer_stats:
                if datacollect.writer_stats[tmprequestendpoint][0] < 200:
                    rand_select_one.append(len(seq_collection) - 1)
        ...
        if len(seq_collection) > 0:
            if not rand_select_one:
                ...
                if random.random() < 0.25:
                    ...
                    rand_int = random.choice(rand_select_one)
                    return seq_collection[rand_int: rand_int + 1]
        else:
            return []
...
def generate_sequences(fuzzing_requests, checkers, fuzzing_jobs=1):
    ...
    while not should_stop:
        ...
        for tmpi in range(len(datacollect.seq_collection_origin)):
            seq_collection.append(datacollect.seq_collection_origin[tmpi])
        ...
        for length in range(min_len, max_len):
            ...
            if len(seq_collection):
                print("\n type: " + str(type(seq_collection[0])))
                for tmpi in range(len(seq_collection)):
                    tmppoint = [r.endpoint_no_dynamic_objects for r in seq_collection[tmpi]]
                    print("what seq: " + str(tmppoint))
                    datacollect.seq_collection_origin.append(seq_collection[tmpi])
                    print("what len: " + str(len(datacollect.seq_collection_origin)))
