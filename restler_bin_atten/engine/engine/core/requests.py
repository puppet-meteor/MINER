...
class Request(object):
    ...
    def render_iter(self, candidate_values_pool, skip=0, preprocessing=False):
        ...
        """
        if (datetime.datetime.now() - datacollect.starttime).seconds > 2*3600:
            datacollect.starttime = datetime.datetime.now()
            datacollect.retrainingmodel()
        ...
        for request_block in definition:
            ...
            elif primitive_type == primitives.FUZZABLE_GROUPNAME:
                try:
            ...
            elif primitive_type == primitives.FUZZABLE_PROJECTNAME:
                try:
                    values = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
                    for lyui in range(100):
                        values.append(values[0] + ''.join(random.sample('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',5)))
                    shuffle(values)
                except:
                    _raise_dict_err(primitive_type, default_val)


            elif primitive_type == primitives.FUZZABLE_USERADDPROJECTNAME:
                try:
                    values = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
                    for lyui in range(100):
                        values.append(values[0] + ''.join(random.sample('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',4)))
                    shuffle(values)
            ...
            elif primitive_type == primitives.FUZZABLE_FORKPROJECTNAME:
                try:
                    values = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
                    for lyui in range(100):
                        values.append(values[0] + ''.join(random.sample('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',6)))
                    shuffle(values)
                except:
                    _raise_dict_err(primitive_type, default_val)


            elif primitive_type == primitives.FUZZABLE_STARPROJECTNAME:
                try:
                    values = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
                    shuffle(values)
                except:
            ...
            elif primitive_type == primitives.FUZZABLE_PROJECT:
                try:
                    values = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
                    shuffle(values)
                except:
        ...
        for ind, values in enumerate(combinations_pool):
            ...
            while random.randint(0, 99) < 5: # 2% to delete one para
                counti = 0
                for lyui in range(len(values)):
                    if values[lyui] == '&':
                        counti += 1
                if counti < 1:
                    break
                selecti = random.randint(1, counti)
                lyui = 0
                whichi = 0
                while lyui < len(values):
                    if values[lyui] == '&':
                        whichi += 1
                        if whichi == selecti:
                            break
                    lyui += 1 
                whichdirect = random.randint(0,1)
                if whichdirect == 0: # backward
                    del values[lyui]
                    lyui -= 1
                    while values[lyui][0] not in ['?','/','&', ' ']:
                        del values[lyui]
                        lyui -= 1
                else:                # forward
                    del values[lyui]
    ...
    def render_iter_lyu(self, candidate_values_pool, skip=0, preprocessing=False):
        """ This is the core method that renders values combinations in a
        request template. It basically is a generator which lazily iterates over
        a pool of possible combination of values that fit the template of the
        requst. Static primitive types (such as, string or delimiters) are
        ...
        for request_block in definition:
            ...
            if primitive_type == primitives.FUZZABLE_UUID4:
                if quoted:
                    values = [(primitives.restler_fuzzable_uuid4, True)]
                    values_default = [(primitives.restler_fuzzable_uuid4, True)]
                else:
                    values = [(primitives.restler_fuzzable_uuid4, False)]
                    values_default = [(primitives.restler_fuzzable_uuid4, False)]
            # Handle enums that have a list of values instead of one default val
            elif primitive_type == primitives.FUZZABLE_GROUP:
                if quoted:
                    values = [f'"{val}"' for val in default_val]
                    values_default = [f'"{val}"' for val in default_val]
                else:
                    values = list(default_val)
                    values_default = list(default_val)
                ...
                try:
                    ...
                    for lyui in range(100):
                        values.append(values[0] + ''.join(random.sample('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',5)))
                    shuffle(values)
                    values_default = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
                except:
                    _raise_dict_err(primitive_type, default_val)
            ...
            # Handle multipart form data
                ...
                try:
                    current_fuzzable_values = candidate_values_pool.\
                        get_candidate_values(primitive_type, request_id=self._request_id, tag=default_val, quoted=quoted)
                    values = [multipart_formdata.render(current_fuzzable_values)]
                    values_default = [multipart_formdata.render(current_fuzzable_values)]
                except primitives.CandidateValueException:
                    _raise_dict_err(primitive_type, default_val)
                except Exception as err:
                    _handle_exception(primitive_type, default_val, err)
            # Handle custom (user defined) static payload
                ...
                try:
                    ...
                    if isinstance(current_fuzzable_values, list):
                        values = current_fuzzable_values
                        values_default = current_fuzzable_values
                    else:
                        values = [current_fuzzable_values]
                        values_default = [current_fuzzable_values]
                except primitives.CandidateValueException:
            ...
            # Handle custom (user defined) static payload with uuid4 suffix
                ...
                try:
                    current_fuzzable_value = candidate_values_pool.\
                        get_candidate_values(primitive_type, request_id=self._request_id, tag=default_val, quoted=quoted)
                    values = [primitives.restler_custom_payload_uuid4_suffix(current_fuzzable_value)]
                    values_default = [primitives.restler_custom_payload_uuid4_suffix(current_fuzzable_value)]
                except primitives.CandidateValueException:
                    _raise_dict_err(primitive_type, default_val)
                except Exception as err:
                    _handle_exception(primitive_type, default_val, err)
                ...
                values = [primitives.restler_refreshable_authentication_token]
                values_default = [primitives.restler_refreshable_authentication_token]
            # Handle all the rest
            else:
                values = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
                values_default = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id, quoted)
            ...
            tmpvalue1 = []
            tmpvalue2 = []
            for lyuii in range(len(values_default)):
                tmpvalue1.append(values_default[lyuii])
            if Settings().fuzzing_mode == 'random-walk' and not preprocessing:
                random.shuffle(values)
            for lyuii in range(len(values_default)):
                tmpvalue2.append(values_default[lyuii])
            if tmpvalue1 != tmpvalue2:
                print("tmpvalue  error!")

            if len(values) == 0:
                _raise_dict_err(primitive_type, current_fuzzable_tag)
            fuzzable.append(values)
            if len(values_default) > 1:
                values_default = [(values_default[0])]
            if isinstance(values_default[0], list):
                values_default = values_default[0]
            fuzzable_default.append(values_default)
        ...
        combinations_pool_default = itertools.product(*fuzzable_default)

        # skip combinations, if asked to
        for _ in range(skip):
            next(combinations_pool)
            next(combinations_pool_default)

        # for each combination's values render dynamic primitives and resolve
        # dependent variables
        for ind, (values, values_default) in enumerate(zip(combinations_pool, combinations_pool_default)):
            values = list(values)
            values_default = list(values_default)

            values = request_utilities.resolve_dynamic_primitives(values, candidate_values_pool)
            values_default = request_utilities.resolve_dynamic_primitives(values_default, candidate_values_pool)

            random.seed(time.time())
            
            while random.randint(0, 99) < 5: # 5% to delete one para
                counti = 0
                for lyui in range(len(values)):
                    if values[lyui] == '&':
                        counti += 1
            ...
            rendered_data = "".join(values)
            yield rendered_data, parser, values, values_default, 0


    #@with_goto
    def render_iter_lyu_atten(self, candidate_values_pool, whetherE, skip=0, preprocessing=False):
        """ This is the core method that renders values combinations in a
        request template. It basically is a generator which lazily iterates over
        a pool of possible combination of values that fit the template of the
        requst. Static primitive types (such as, string or delimiters) are
        ...
        for ind, (values, values_default) in enumerate(zip(combinations_pool, combinations_pool_default)):
            ...
            if whetherE == 3:
                if random.random() < 0.5:
                    whetherE = 2 # jump generation mutation
                else:
                    whichG = 0 # generate the mutation of 'P'
            if whetherE == 2:
                if random.random() < 0.5:
                    whichG = 2  # jump generation mutation
                else: 
                    whetherE = 1 # use generation mutation and select 'E' or 'P'
            ...
            tmpvalue = []
            values_default_data = "".join(values_default)
            if whichG == 1:
                if values_default_data in datacollect.requestid: 
                    tmprequesetid = str(datacollect.requestid[values_default_data]) + 'E'
                    if  tmprequesetid  in datacollect.mutationlist.keys():
                        changekey = 0
                        tmpvalue = []
                        for lyukkk in range(len(values_default)):
                            if changekey == 1:
                                changekey = 0
                                tmpvalue.append(values_default[lyukkk] + ''.join(random.sample('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',5)))
                            else:
                                tmpvalue.append(values_default[lyukkk])
                            if values_default[lyukkk] == 'name=' or values_default[lyukkk] == 'path=':
                                changekey = 1
                        try:
                            selectone = random.randint(0, len(datacollect.mutationlist[tmprequesetid]))
                            generationmutation = datacollect.mutationlist[tmprequesetid][selectone]

                            for (tmpnode, tmpoperator) in generationmutation:
                                if tmpoperator == 'deletebylyu':
                                    if tmpnode in tmpvalue:
                                        tmploc = tmpvalue.index(tmpnode)
                                        del tmpvalue[tmploc]
                                        while tmpvalue[lyui][0] not in ['?','/','&', ' ']:
                                            del tmpvalue[tmploc]
                                else:
                                    if tmpnode in tmpvalue:
                                        tmploc = tmpvalue.index(tmpnode) + 1
                                        while tmpvalue[tmploc][0] not in ['?','/','&', ' ']:
                                            del tmpvalue[tmploc]
                                        tmpoperator = [tmpoperator]
                                        tmpvalue[tmploc:tmploc] = tmpoperator
                                    else: 
                                        for lyui in range(len(tmpvalue)):
                                            if tmpvalue[lyui][0] == ' ':
                                                break
                                        if lyui < len(tmpvalue) -1:
                                            tmpmutation = ['&']
                                            tmpmutation.append(tmpnode)
                                            tmpmutation.append(tmpoperator)
                                            tmpvalue[lyui:lyui] = tmpmutation
                        except:
                            tmpvalue = []
                            for lyukkk in range(len(values)):
                                tmpvalue.append(values[lyukkk])
                            whichG = 2
            ...
            if whichG == 0:
                if values_default_data in datacollect.requestid:
                    tmprequesetid = str(datacollect.requestid[values_default_data]) + 'P'
                    if  tmprequesetid  in datacollect.mutationlist.keys():
                        changekey = 0
                        tmpvalue = []
                        for lyukkk in range(len(values_default)):
            ...
            if whichG == 1 or whichG == 0:
                if tmpvalue == []:
                    for lyukkk in range(len(values)):
                        tmpvalue.append(values[lyukkk])
                rendered_data = "".join(tmpvalue)
                yield rendered_data, parser, tmpvalue, values_default, 1
            else:    
                rendered_data = "".join(values)
                yield rendered_data, parser, values, values_default, 0

    #@with_goto
    def render_iter_lyu_atten2(self, candidate_values_pool, whetherE, skip=0, preprocessing=False):
        """ This is the core method that renders values combinations in a
        request template. It basically is a generator which lazily iterates over
        a pool of possible combination of values that fit the template of the
        requst. Static primitive types (such as, string or delimiters) are
        ...
        for ind, (values, values_default) in enumerate(zip(combinations_pool, combinations_pool_default)):
            ...
            if datacollect.whether_start_mutation_list == 1:
                if random.random() < 0.5:
                    whichG = 2 # jump generation mutation
                else:
                    whichG = 0 # generate the mutation of 'P'
            else:
                #selection probability based on score
                score_gen = (datacollect.seq_gen200*3 + datacollect.seq_gen500 - datacollect.seq_gen400*2)*datacollect.seq_nogen_num
                score_nogen = (datacollect.seq_nogen200*3 + datacollect.seq_nogen500 - datacollect.seq_nogen400*2)*datacollect.seq_gen_num
                if score_gen <=0 or score_nogen<=0:
                    whichG = 0
                else:
                    if random.randint(0, score_gen + score_nogen - 1) < score_gen:
                        whichG = 0
                    else:
                        whichG = 2
                                ...
                                if tmpoperator == 'deletebylyu':
                                    ...
                                    if tmpnode in tmpvalue:
                                        ...
                                        if lyui < len(tmpvalue) - 1:
                                            ...
                                            tmpvalue[lyui:lyui] = tmpmutation
                            seq_which = 1
                            datacollect.seq_gen_num += 1
                        except:
                            tmpvalue = []
                            for lyukkk in range(len(values)):
                                tmpvalue.append(values[lyukkk])
                            seq_which = 2
                            datacollect.seq_nogen_num += 1
                            whichG = 2
            ...
            if  whichG == 0:
                if tmpvalue == []:
                    for lyukkk in range(len(values)):
                        tmpvalue.append(values[lyukkk])
                rendered_data = "".join(tmpvalue)
                yield rendered_data, parser, tmpvalue, values_default, 1, seq_which
            else:    
                rendered_data = "".join(values)
                yield rendered_data, parser, values, values_default, 0, 2 


    #@with_goto
    def render_iter_lyu_atten3(self, candidate_values_pool, skip=0, preprocessing=False):
        def _raise_dict_err(type, tag):
            logger.write_to_main(
                f"Error for request {self.method} {self.endpoint_no_dynamic_objects}.\n"
                f"{type} exception: {tag} not found.\n"
        ...
        for ind, (values, values_default) in enumerate(zip(combinations_pool, combinations_pool_default)):
            ...
            if tmpvalue == []:
                ...
                try:
                    tmpkey1 = [key for key in list(datacollect.mutationtrainingset) if key[-1]=='E']
                    tmpkey = ''
                    if tmpkey1:
                        tmpkey = random.choice(tmpkey1)
                    else:
                        tmpkey2 = ''
                        if values_default_data in datacollect.requestid:
                            tmprequesetid = str(datacollect.requestid[values_default_data]) + 'P'
                            tmpkey2 = [key for key in list(datacollect.mutationtrainingset) if tmprequesetid  not in key]
                        else:
                            tmpkey2 = list(datacollect.mutationtrainingset)
                        if tmpkey2 != '':
                            tmpkey = random.choice(tmpkey2)
                    if tmpkey != '':
                        if len(datacollect.mutationtrainingset[tmpkey]) > 1:
                            tmpmutation1 = random.choice(list(datacollect.mutationtrainingset[tmpkey]))                
                            for lyui in range(len(tmpvalue)):
                                if tmpvalue[lyui][0] == ' ':
                                    break
                            if lyui < len(tmpvalue) - 1:
                                tmpmutation2 = ['&']
                                for key3 in tmpmutation1:
                                    tmpmutation2.append(key3)
                                tmpvalue[lyui:lyui] = tmpmutation2
                except:
                    tmpkey = ''
    ...
    def render_current_lyu(self, candidate_values_pool, whetherE, preprocessing=False):
        """ Renders the next combination for the current request.
        ...
        random.seed(datetime.datetime.now())
        if random.random() < 0.5:
            return next(self.render_iter_lyu(candidate_values_pool,
                                skip=self._current_combination_id - 1,
                                preprocessing=preprocessing))
        else:
            return next(self.render_iter_lyu_atten(candidate_values_pool,
                                whetherE,
                                skip=self._current_combination_id - 1,
                                preprocessing=preprocessing))


    def render_current_lyu2(self, candidate_values_pool, whetherE, preprocessing=False):
        """ Renders the next combination for the current request.
        ...
        random.seed(datetime.datetime.now())
        return next(self.render_iter_lyu_atten2(candidate_values_pool,
                                whetherE,
                                skip=self._current_combination_id - 1,
                                preprocessing=preprocessing))


    def render_current_lyu3(self, candidate_values_pool, preprocessing=False):
        """ Renders the next combination for the current request.
        ...
        """
        if (datetime.datetime.now() - datacollect.starttime).seconds > 2*3600: #43200:
            datacollect.starttime = datetime.datetime.now()
            datacollect.retrainingmodel()


        random.seed(datetime.datetime.now())
        return next(self.render_iter_lyu_atten3(candidate_values_pool,
                                skip=self._current_combination_id - 1,
                                preprocessing=preprocessing))
