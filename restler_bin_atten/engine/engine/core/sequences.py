...
class Sequence(object):
    ...
    def render(self, candidate_values_pool, lock, preprocessing=False, postprocessing=False):
        ...
        for rendered_data, parser, list_values, list_values_default, whethergen  in\
                request.render_iter_lyu(candidate_values_pool, skip=request._current_combination_id,
            ...
            datacollect.seq_if = 1
            has_500 = False
            has_400 = False
            has_200 = True
            for i in range(len(self.requests) - 1):
                prev_request = self.requests[i]
                if datacollect.whether_start_mutation_list == 0:
                    prev_rendered_data, prev_parser, prev_list_values, prev_list_values_default, whethergen =\
                    prev_request.render_current_lyu(candidate_values_pool, 0, preprocessing=preprocessing)    # render the last request  why again?   generate again to solve the dependencies?   avoid the same prev_request??
                else:
                    prev_rendered_data, prev_parser, prev_list_values, prev_list_values_default, whethergen, datacollect.seq_which =\
                    prev_request.render_current_lyu2(candidate_values_pool, 0, preprocessing=preprocessing)
                ...
                if prev_status_code[0] == '5':
                    has_500 = True
                    has_200 = False
                elif prev_status_code[0] == '4':
                    has_400 = True
                    has_200 = False

                if has_500 and not has_400:
                    datacollect.seq_part_500 += 1
                elif has_400 and not has_500:
                    datacollect.seq_part_400 += 1
                elif has_400 and has_500:
                    datacollect.seq_part_non200 += 1
                elif has_200:
                    datacollect.seq_part_200 += 1

                if '?' in prev_list_values_default:  # otherwise there is no node to mutate 
                    if prev_status_code[0] == '2':
                        if whethergen == 1:
                        ...
                        tmp_prev_list_values_default = "".join(prev_list_values_default)
                        if len(datacollect.requestid) == 0:
                            datacollect.requestid[tmp_prev_list_values_default] = 0
                        elif tmp_prev_list_values_default  not in datacollect.requestid:
                            datacollect.requestid[tmp_prev_list_values_default] = max(datacollect.requestid.values()) + 1
                    
                        #  node  + mutation results
                        countid = datacollect.countid
                        request_name = datacollect.requestid[tmp_prev_list_values_default]
                        start_loc = datacollect.reverseindex(prev_list_values_default, '?')
                        statelyu = 0

                        tmp_status_code = ''
                        if prev_status_code[0] == '2':
                            tmp_status_code = 'P'
                        elif prev_status_code[0] == '5':
                            tmp_status_code = 'E'
                        for lyui in range(start_loc, len(prev_list_values_default)):
                            if prev_list_values_default[lyui][0] == ' ':
                                break   # finish find node + mutation results in _default

                            if  prev_list_values_default[lyui] in ['?', '&']:
                                statelyu = 0
                                continue
                            if statelyu == 0:
                                statelyu = 1
                                tmpnode = prev_list_values_default[lyui]
                                if tmpnode not in prev_list_values:
                                    ...
                                    tmplist = []
                                    tmplist2 = []
                                    start_loc2 = prev_list_values.index(tmpnode)
                                    for lyuj in range(start_loc2 + 1, len(prev_list_values)):
                                        if prev_list_values[lyuj][0] in ['&', ' ']:
                                            break
                                        tmplist.append(prev_list_values[lyuj])
                                    for lyuj in range(lyui + 1, len(prev_list_values_default)):
                                        if prev_list_values_default[lyuj][0] in ['&', ' ']:
                                            break
                                        tmplist2.append(prev_list_values_default[lyuj])
                                    if tmplist != tmplist2:
                                        tmpset = [countid, request_name]
                                        tmpset.append(tmpnode)  # node
                                        ...
                                        datacollect.trainingset.append(tmpset)
                                        #continue 
                        start_loc = datacollect.reverseindex(prev_list_values, '?')
                        statelyu = 0
                        for lyui in range(start_loc, len(prev_list_values)):
                            if prev_list_values[lyui] == ' ':
                                break  # finish find node + mutation results in prev_list_values
 
                            if prev_list_values[lyui] in ['?', '&']:
                                statelyu = 0 
                                continue
                            if statelyu == 0:
                                statelyu = 1                                
                                tmpnode = prev_list_values[lyui]
                                if tmpnode not in prev_list_values_default:
                                    tmplist = []
                                    for lyuj in range(lyui + 1, len(prev_list_values)):
                                        if prev_list_values[lyuj][0] in ['&', ' ']:
                                            break
                                        tmplist.append(prev_list_values[lyuj])
                                    tmpset = [countid, request_name]
                ...
                self.status_codes.append(status_codes_monitor.RequestExecutionStatus(timestamp_micro,
                                                                ...
                                                                False))
            datacollect.seq_if = 0

            if has_500 and not has_400:
                datacollect.seq_overall_500 += 1
            elif has_400 and not has_500:
                datacollect.seq_overall_400 += 1
            elif has_400 and has_500:
                datacollect.seq_overall_non200 += 1
            elif has_200:
                datacollect.seq_overall_200 += 1
